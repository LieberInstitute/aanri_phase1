...
for(i in 1:nrow(dmr_local)){
	...
	var1 <- signif(var(ances[,idx]/2),2)
	var2 <- signif(var(ancesg$Afr),2)
	main <- paste(rownames(dmr_local)[i],
	    # beta, se and t
		paste0("local: beta=", round(res[idx,4],2),",se=",round(res[idx,5],2),",t=",round(res[idx,6],2),",var=",var1),
		paste0("global: beta=", round(dmr_global[idx2,6],2),",se=",round(dmr_global[idx2,7],2),",t=",round(dmr_global[idx2,8],2),",var=",var2),sep="\n")
	plot(ances[,idx]/2,p[,idx],xlim=c(0,1),col="red",xlab="ancestry",ylab="DNAm",main=main,cex.main = 0.8)
...
# GO analysis
dmr <- plyranges::as_granges(res[res$fdr < 0.1,1:3])
bg <- plyranges::as_granges(res[,1:3])
go <- great(dmr,"GO:BP","RefSeq:hg38", background=bg)
tb  <- getEnrichmentTable(go)
write.csv(tb,"./out/dmr_great_fdr0.1_bg.csv")
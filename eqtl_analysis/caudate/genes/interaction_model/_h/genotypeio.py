...
import sys, gzip, queue, threading
...
class PlinkReader(object):
    def __init__(self, plink_prefix_path,fam_id="iid", select_samples=None,
                 include_variants=None, exclude_variants=None, exclude_chrs=None,
        ...
        self.sample_ids = self.fam[fam_id].tolist()
    ...
    def get_genotype(self, variant_id, fam_id='iid', sample_ids=None, impute=False,
                     verbose=False, dtype=np.int8):
        """Load genotypes for a single variant ID as pd.Series"""
        g,_ = self.get_genotypes([variant_id], sample_ids=sample_ids, impute=impute,
                                 verbose=verbose, dtype=dtype)
        if sample_ids is None:
            return pd.Series(g[0], index=self.fam[fam_id], name=variant_id)
        else:
            return pd.Series(g[0], index=sample_ids, name=variant_id)

    def load_genotypes(self, fam_id='iid'):
        """Load all genotypes into memory, as pd.DataFrame"""
        return pd.DataFrame(self.bed.compute(), index=self.bim['snp'], columns=self.fam[fam_id])
...
def load_genotypes(plink_prefix_path, fam_id="iid", select_samples=None, dtype=np.int8):
    ...
    if all([os.path.exists(f"{plink_prefix_path}.{ext}" for ext in ['bed', 'bim', 'fam'])]):
        pr = PlinkReader(plink_prefix_path, fam_id=fam_id, select_samples=select_samples,
                         dtype=dtype)
        df = pr.load_genotypes(fam_id)

...
parser$add_argument("-t", "--tissue", type="character", default="dentateGyrus",
...
p       <- read.table(pfile, comment.char="", header=TRUE)
                                        ...
                                        # get top 25 from caudate
caud  <- read.table("caudate/out/eqtls_top25.tsv", sep="\t",
                    comment.char="", header=TRUE)
                                        # gene meta
efile <- paste0(outdir, "/eqtls.tsv")
e     <- read.table(efile, comment.char="", sep="\t", header=TRUE) %>%
    filter(gene_id %in% caud$gene_id)
...
p    <- p %>% filter(gene_id %in% caud$gene_id) %>%

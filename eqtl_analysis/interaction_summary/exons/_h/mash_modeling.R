...
load_top_eqtls <- function(){
    fn = "top_eqtls_AA_interaction.txt.gz"
...
run_mashr <- function(SEED, percentage){
    ...
    random.subset <- sample(1:nrow(bhat), rand_n)
                                        # Strong subset
    ## m.1by1 = mash_1by1(mash_set_data(bhat, shat)) # Extract with meta-analysis
    ## strong.subset = get_significant_results(m.1by1, 0.05)
    ## rm(m.1by1); gc(verbose=TRUE)
    strong.subset <- match(load_top_eqtls()$effect, # Use top cis-eQTLs
                           rownames(shat)) %>%
        na.omit # eQTL may be specific to one brain region
    ## m.1by1 can't be done with the number of eQTLs!

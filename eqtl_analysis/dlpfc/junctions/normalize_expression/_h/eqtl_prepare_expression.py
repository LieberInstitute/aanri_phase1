...
def gtf_to_bed(annotation_gtf, feature='gene', exclude_chrs=[]):
    gtf0 = get_gtf(annotation_gtf)
    gtf = gtf0[gtf0["feature"] == feature]
    gtf = gtf[["gene_id", "gene_name", "transcript_id", "exon_id",
               "seqname", "start", "end", "strand"]]
    if feature == 'gene':
        bed_df = pd.DataFrame(data={"chr":np.array(gtf.seqname),
                                    'start':np.array(gtf.start),
                                    'end':np.array(gtf.start)+1,
                                    'gene_id':np.array(gtf.gene_id)},
                              columns=['chr', 'start', 'end', 'gene_id'],
                              index=np.array(gtf.gene_id)).drop_duplicates()
    elif feature == "transcript":
        bed_df = pd.DataFrame(data={"chr":np.array(gtf.seqname),
                                    'start':np.array(gtf.start),
                                    'end':np.array(gtf.start)+1,
                                    'gene_id':np.array(gtf.transcript_id)},
                              columns=['chr', 'start', 'end', 'gene_id'],
                              index=np.array(gtf.transcript_id))\
                   .drop_duplicates()
    else:
        bed_df = pd.DataFrame(data={"chr":np.array(gtf.seqname),
                                    'start':np.array(gtf.start),
                                    'end':np.array(gtf.start)+1,
                                    'gene_id':np.array(gtf.exon_id)},
                              columns=['chr', 'start', 'end', 'gene_id'],
                              index=np.array(gtf.exon_id)).drop_duplicates()
    # drop rows corresponding to excluded chromosomes
    mask = np.ones(len(gtf.seqname), dtype=bool)
...
#     bed_df = pd.DataFrame(data={'chr':chrom, 'start':start,
#                                 'end':end, 'gene_id':gene_id},
#                           columns=['chr', 'start', 'end', 'gene_id'],
#                           index=gene_id)
#     # drop rows corresponding to excluded chromosomes
...
def write_bed(bed_df, output_name):
    ...
    subprocess.check_call('bgzip -f '+output_name, shell=True,
                          executable='/bin/bash')
    subprocess.check_call('tabix -f '+output_name+'.gz', shell=True,
                          executable='/bin/bash')
...
if __name__=='__main__':
    ...
    if args.bed_file is None:
        bed_template_df = gtf_to_bed(args.annotation_gtf, args.feature)
    else:

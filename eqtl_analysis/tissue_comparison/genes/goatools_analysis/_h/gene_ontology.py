...
def get_ancestry_deg(tissue):
    ...
    df = df[(df["Tissue"] == tissue.replace("_", " ")) &
            (df["Type"] == "Gene")].copy()
...
@lru_cache()
def get_upregulated(tissue):
    df = get_ancestry_deg(tissue)
    return df[(df["posterior_mean"] > 0)].copy()


@lru_cache()
def get_downregulated(tissue):
    df = get_ancestry_deg(tissue)
    return df[(df["posterior_mean"] < 0)].copy()


@lru_cache()
def overlapping_features(tissue, direction):
    ...
    if direction == "ALL":
        ...
        deg = set(get_upregulated(tissue).Effect)
        label = "%s_%s" % (label0, direction.lower())
    else:
        deg = set(get_downregulated(tissue).Effect)
        label = "%s_%s" % (label0, direction.lower())
    ...
    return df[(df["gene_id"].isin(shared_features))]\
        .drop_duplicates(subset="gene_id"), label
...
def run_goea(tissue, direction):
    df, label = overlapping_features(tissue, direction)

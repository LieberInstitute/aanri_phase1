...
IGP.clusterRepro <- function(Data, Centroids) {
    Result <- c(); Class <- rep(NA, ncol(Data))
    names(Class) <- colnames(Data); Features <- rownames(Data)
    Features <- Features[which(Features %in% rownames(Centroids))]
    Data <- Data[Features,]; Centroids <- Centroids[Features,]
...
clusterRepro <- function(Centroids, New.data, Number.of.permutations) {
    Result <- c(); Features <- rownames(New.data)
    Features <- Features[which(Features %in% rownames(Centroids))]
    ...
    IGP <- c(); Size <- c(); SVD <- svd(Centroids)
    Centroids.prime <- Centroids%*%SVD$v
    df <- foreach (i=1:Number.of.permutations, .combine=rbind) %dopar% {
        ...
        data.frame("IGP"=IGP, "Size"=Size)
    }
    Result$p.value <- c(NA, length(Actual.Size))
    Result$Number <- c(NA, length(Actual.Size))
    for (i in 1:length(Actual.Size)) {
        if (length(which(df$Size == Actual.Size[i])) == 0 || Actual.Size[i] == 0) {
            Result$p.value[i] <- NA;
            Result$Number[i]  <- 0;
        } else {
            Same.size         <- df$IGP[which(df$Size == Actual.Size[i])];

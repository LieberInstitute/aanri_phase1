...
prepare_data <- function(){
                                        # Load sample data
    fn1 <- here("differential_analysis/dentateGyrus/_m/",
                "genes/voomSVA.RData")
    ...
    fn2 <- here("differential_analysis/dentateGyrus/",
...
plot_power_parameter <- function(datExpr, plot_filename){
                                        ...
                                        # powers
    powers <- c(seq(1, 10, 1),  seq(12, to=20, by=2))
...
construct_network <- function(softPower){
    ...
    cor <- WGCNA::cor
    net = blockwiseModules(datExpr, mergeCutHeight = 0.3,

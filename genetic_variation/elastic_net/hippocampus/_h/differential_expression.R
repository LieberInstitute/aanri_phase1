...
load_model <- function(feature){
    fn <- here("differential_analysis/hippocampus/_m/", feature,"/voomSVA.RData")
    load(fn)
    ...
    return(v$design %>% as.data.frame %>% rownames_to_column("RNum") %>%
        ...
        select(-RNum) %>% as.matrix)
}

fit_model <- function(feature){
    # Load data and design matrix
    norm_file <- paste0("../../_m/",feature,".predicted_expression.hippocampus.txt.gz")
    norm_expr <- data.table::fread(norm_file) %>% column_to_rownames("gene_id")
    mod       <- load_model(feature)
    # Fit model
    sample_ids   <- intersect(rownames(mod), colnames(norm_expr))
    logCPM       <- norm_expr %>% select(any_of(sample_ids))

...
def get_residualized(tissue, feature):
    ...
    res_file = here("differential_analysis/%s/" % tissue,
                    "_m/%s/residualized_expression.tsv" % feature)
    res_df = pd.read_csv(res_file, sep='\t', index_col=0)
    # Load sample phenotypes
    pheno_file = here("input/phenotypes/merged/_m/merged_phenotypes.csv")
    pheno_df = pd.read_csv(pheno_file)\
                 .loc[:, ["BrNum", "RNum"]]\
                 .set_index("RNum").to_dict()
    return res_df.rename(columns=pheno_df["BrNum"])
...
def subset_phenotypes(tissue, feature):
    ...
    shared_samples = set(res_df.columns) & set(phenotype_df.columns)
    shared_features = set(res_df.index) & set(phenotype_df.index)
    phenotype_pos_df["gene_id"] = phenotype_pos_df.index
    phenotype_df = res_df.loc[shared_features, shared_samples].sort_index()

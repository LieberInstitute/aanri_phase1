...
extract_de <- function(contrast, label, efit, feature){
    top <- limma::topTable(efit, coef=contrast,
    ...
    top %>% rownames_to_column("gene_id") %>%
        data.table::fwrite(file=paste0(feature, "/diffExpr_",
                                       label, "_full.txt"), 
                           sep='\t', row.names=FALSE)
    top.fdr %>% rownames_to_column("gene_id") %>%
        data.table::fwrite(file=paste0(feature, "/diffExpr_",
                                       label, "_FDR05.txt"), 
                           sep='\t', row.names=FALSE)
}
...
load_model <- function(feature){
    fn <- here("differential_analysis/caudate/_m/",
               feature,"/voomSVA.RData")
    load(fn)
    design0 <- v$targets %>% select(BrNum, RNum)
    return(v$design %>% as.data.frame %>%
           rownames_to_column("RNum") %>%
           inner_join(design0, by="RNum") %>%
           column_to_rownames("BrNum") %>%
        select(Intercept, EA) %>% as.matrix)
}

get_background <- function(feature){
    fn <- here("differential_analysis/caudate/_m/",
               feature,"/voomSVA.RData")
    load(fn)
    return(rownames(v$genes))
}

fit_model <- function(feature){
    # Load data and design matrix
    res_file <- paste0("../../_m/",feature,
                       ".predicted_expression.caudate.txt.gz")
    res_expr <- data.table::fread(res_file) %>%
        filter(gene_id %in% get_background(feature)) %>%

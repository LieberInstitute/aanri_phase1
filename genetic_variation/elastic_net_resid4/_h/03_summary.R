...
tissue  <- args$tissue
                                        ...
                                        # results of normalized expression
files <- list.files(paste(tissue, feature, "out", sep="/"),
                    "p.csv", full.names=TRUE)
dat   <- dplyr::bind_rows(purrr::map(files, read.csv))
m1    <- apply(dat[,-(1:2)],1,function(x){mean(x^2,na.rm=T)})
...
res     <- data.frame(gene=dat$genes,r2_norm=m1,r2_residual=m2)

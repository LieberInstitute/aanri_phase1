...
def run_rf(X_train, X_test, Y_train, Y_test, fold_num, outdir):
    """
    ...
    output['train_oob_score_r2_all_features'] = pfirst[1]
    output['train_oob_score_mse_all_features'] = pfirst[2]
    output['train_oob_score_evar_all_features'] = pfirst[3]
    output['train_oob_score_mse'] = dRFEtools.oob_score_mse(regr, Y_train)
    output['train_oob_score_r2'] = dRFEtools.oob_score_r2(regr, Y_train)
    output['train_oob_score_evar'] = dRFEtools.oob_score_evar(regr, Y_train)
...
def run_by_feature(args):
    ...
    mkdir_p(outdir)
    df = pd.read_csv("%s/snps_methylation.csv" % args.fn, index_col=0)
    ...
    fields = ['n_features_all_features', 'train_oob_score_mse_all_features',
              'train_oob_score_r2_all_features', 'train_oob_score_evar_all_features',
              'n_features', 'train_oob_score_r2', 'train_oob_score_mse',
              'train_oob_score_evar', 'test_score_r2', 'test_score_mse',
              'test_score_evar', 'n_redundant', 'test_score_r2_redundant',
              'test_score_mse_redundant', 'test_score_evar_redundant']
    fold = 0
    df_dict = pd.DataFrame()
    with open('%s/rf_10folds.txt' % (outdir), 'w') as f:
        print("\t".join(['fold'] + fields), file=f, flush=True)
        for train_index, test_index in skf.split(X, y):
            #print("TRAIN:", train_index, "\nTEST:", test_index, "\nFold:", fold)
            X_train, X_test = X.iloc[train_index, :], X.iloc[test_index, :]
            Y_train, Y_test = Y[train_index], Y[test_index]
            o, df_elim = run_rf(X_train, X_test, Y_train, Y_test, fold, outdir)

...
extract_de <- function(contrast, label, efit, feature){
    ...
    top.fdr <- filter(top, adj.P.Val<=0.05)
...
get_hla_pcs <- function(){
    fn <- here("input/genotypes/chr6_qc/hla_imputation",
               "post_imputationQC/extract_hla/_m/HLA_LIBD_AA.eigenvec")
    return(data.table::fread(fn) |>
           rename_all(list(~str_replace_all(., 'PC', 'hPC'))))
}
...
prep_data <- function(feature){
    ...
    tx_file = here("input/counts/_m/transcripts_counts/caudate_counts.txt")
    ...
    if(exists("rse_gene")){
        ...
        counts  <- data.table::fread(tx_file) |>
            ...
            select(any_of(colnames(rse_df))) |> as.matrix()
        keepIdx <- which(colnames(rse_df) %in% colnames(counts))
        rse_df  <- rse_df[, keepIdx]
        assays(rse_df)$counts = counts
    ...
    pheno <- colData(rse_df) |> as.data.frame() |> 
        ...
        inner_join(get_hla_pcs(), by=c("BrNum"="#IID"))
                                        # Generate DGE list
...
get_qSVs <- function(){
    qsv_file = here("input/phenotypes/_m/qSV_caudate.csv")
    return(data.table::fread(qsv_file) |>
...
qSV_model <- function(feature){
    ...
    mod = model.matrix(~Eur + Sex + Age + mitoRate + rRNA_rate + 
                           totalAssignedGene + overallMapRate +
    ...
    # Load qSV
    modQsva <- as.data.frame(mod) |> tibble::rownames_to_column() |>
        inner_join(get_qSVs(), by=c("rowname"="V1")) |> 

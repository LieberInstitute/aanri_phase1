...
def merge_data(feature):
    return pd.merge(get_mash_cell(feature), get_mash_obs(feature),
...
def corr_beta(fnc, tissue, feature, lfsr = 0.05):
    return spearmanr(fnc(tissue, feature, lfsr)[f"{tissue}_cell"],
...
def plotNsave_corr(fnc, tissue, feature, label):
    ...
    ## Plotting
    xlab = "Observed\n(Effect Size)"
    ylab = "Glial cell correction\n(Effect Size)"
    fn = paste(feature, "effectsize_scatter_cellVobs",
               tolower(tissue), label, sep=".")
    pp = ggscatter(df, x=paste0(tissue,"_obs"), y=paste0(tissue,"_cell"),

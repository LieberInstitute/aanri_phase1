...
load_DEG_enrichment <- function(){
    return(data.table::fread("clincial_phenotypes_enrichment_analysis_DEGs_tx.tsv"))
}
memDEG <- memoise::memoise(load_DEG_enrichment)

load_TWAS_enrichment <- function(){
    return(data.table::fread("clincial_phenotypes_enrichment_analysis_TWAS_tx.tsv") %>%
...
gen_data <- function(fnc){
    ...
    dt <- fnc() %>% mutate_if(is.character, as.factor) %>%
        mutate(Tissue=fct_relevel(Tissue, rev), `-log10(FDR)`= -log10(FDR),
               `OR Percentile`= OR / (1+OR), p.fdr.sig=FDR < 0.25,
               `log2(OR)` = log2(OR+err),
               p.fdr.cat=cut(FDR, breaks=c(1,0.05,0.01,0.005,0),
                             labels=c("<= 0.005","<= 0.01","<= 0.05","> 0.05"),
                             include.lowest=TRUE)) %>%
        mutate(across(Direction, factor,
                      levels=c("All", "Increased in AA", "Decreased in AA")))
    return(dt)
}
...
plot_tile <- function(fnc, label, w, h){
    ...
    save_plot(tile_plot, paste0("tileplot_enrichment_",
                                tolower(label), "_tx"), w, h)
}

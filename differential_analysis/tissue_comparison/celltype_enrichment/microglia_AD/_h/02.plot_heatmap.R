...
load_enrichment <- function(){
    return(data.table::fread("celltype_enrichment.tsv"))
}
...
gen_data <- function(){
    ...
    dt <- memENRICH() |> 
        ...
        mutate(MStates=paste0("MG", Cluster),
               `-log10(FDR)`= -log10(FDR),
               ...
               p.fdr.cat=cut(FDR, breaks=c(1,0.05,0.01,0.005,0),
                             ...
                             include.lowest=TRUE)) |>
        mutate_at("MStates", as.factor)
    mstates <- paste0("MG", seq(0,12))[!grepl("MG9", paste0("MG", seq(0,12)))]
    dt$MStates <- factor(dt$MStates, levels=mstates)
    return(dt)
}
...
plot_tile <- function(label, w, h){
    ...
    tile_plot <- memDF() |>
        mutate(MStates=forcats::fct_relevel(MStates, rev)) |>
        ggplot(aes(y = MStates, x = Tissue, fill = `log2(OR)`,
                   label = ifelse(p.fdr.sig,
                                  format(round(`-log10(FDR)`,1), nsmall=1), ""))) +
        xlab('Ancestry-Associated DEGs') + ylab("Microglial States") +
        facet_wrap(.~Direction) + geom_tile(color = "grey") +

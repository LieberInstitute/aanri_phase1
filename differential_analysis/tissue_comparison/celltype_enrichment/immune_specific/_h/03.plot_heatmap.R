...
load_celltype_enrichment <- function(){
    return(data.table::fread("celltype_enrichment_analysis.txt"))
}
...
plot_tile <- function(label, w, h){
    ...
    tile_plot <- memDF() |>
        ...
        xlab('Ancestry-Related DEGs') + ylab(label) + facet_grid(.~Direction) +
...
## Run script
plot_tile("Immune_Celltypes", 12, 8)

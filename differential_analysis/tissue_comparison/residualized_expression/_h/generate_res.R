...
get_model <- function(){
    covs = c('Sex', 'Age', 'mitoRate', 'rRNA_rate', 'totalAssignedGene',
             'overallMapRate', 'Region', "Race", 'BrNum')
    adjust_covs = covs[1:6]
    random_effect = covs[9]
...
get_null <- function(){
    ...
    adjust_covs = covs[1:6]
    random_effect = covs[9]
    null_form <- glue("~ ", glue_collapse(adjust_covs[2:6], sep = " + ")) %>%
        glue(" + (1|{random_effect}) + (1|Sex)")
    return(null_form)
}
...
prep_data <- function(feature){
    ...
    for(region in c('dlpfc', 'caudate', 'hippocampus', 'gyrus')){
        ...
        pheno = colData(rse_df) %>% as.data.frame %>%
            inner_join(fread(ancestry), by=c("BrNum"="id", "Race"="group"))

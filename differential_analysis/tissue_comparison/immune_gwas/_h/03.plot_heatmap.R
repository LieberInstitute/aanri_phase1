...
load_enrichment <- function(){
    return(data.table::fread("immune_GWAS_enrichment.tsv"))
}
...
gen_data <- function(){
    ...
    dt <- memENRICH() |> mutate_if(is.character, as.factor) |>
        mutate(Dataset=forcats::fct_relevel(Dataset, rev),
               `-log10(FDR)`= -log10(FDR),
...
plot_tile <- function(label, w, h){
    ...
    tile_plot <- memDF() |>
        ggplot(aes(y = Dataset, x = Tissue, fill = `log2(OR)`,
                   label = ifelse(p.fdr.sig,
                                  format(round(`-log10(FDR)`,1), nsmall=1), ""))) +
        xlab('Ancestry-Associated DEGs') + ylab("Immune GWAS") +

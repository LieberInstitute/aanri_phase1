...
get_sig_genes <- function(){
    ...
    return(data.table::fread(fn) %>% dplyr::filter(Type == "Gene"))
}
memGENES <- memoise::memoise(get_sig_genes)

prep_heatmap <- function(){
    caudate <- memGENES() %>% dplyr::filter(Tissue == "Caudate")
    gyrus <- memGENES() %>% dplyr::filter(Tissue == "Dentate Gyrus")
    dlpfc <- memGENES() %>% dplyr::filter(Tissue == "DLPFC")
    hippo <- memGENES() %>% dplyr::filter(Tissue == "Hippocampus")
    lt = list(Caudate = caudate$Effect, `Dentate Gyrus`=gyrus$Effect,
              DLPFC=dlpfc$Effect, Hippocampus=hippo$Effect)
    m = make_comb_mat(lt)
    return(m)
}

...
prep_data <- function(feature){
    counts_lt = list("genes"=here("input/counts/_m",
                                  paste0("dlpfc_ribozero_brainseq_phase2_",
                                         "hg38_rseGene_merged_n453.rda")),
                     "transcripts"=here("input/counts/_m",
                                        paste0("dlpfc_ribozero_brainseq_phase2_",
                                               "hg38_rseTx_merged_n453.rda")),
                     "exons"=here("input/counts/_m",
                                  paste0("dlpfc_ribozero_brainseq_phase2_",
                                         "hg38_rseExon_merged_n453.rda")),
                     "junctions"=here("input/counts/_m",
                                      paste0("dlpfc_ribozero_brainseq_phase2_",
                                             "hg38_rseJxn_merged_n453.rda")))
...
get_local_ancestry <- function(feature, feature_id){
    afile = paste0("../../_m/dlpfc/",feature,"/ancestry/",
                   feature_id,"_AFR")
...
feature <- args$feature; n <- args$chunks; ID <- args$sge_id
                                        # Chunk data for loop
ids <- gsub("_AFR", "",
            list.files(paste0("../../_m/dlpfc/",feature,"/ancestry"),
                       pattern="\\_AFR$")) |> sort()

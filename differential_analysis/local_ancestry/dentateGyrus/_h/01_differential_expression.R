...
prep_data <- function(feature){
    counts_lt = list("genes"=here("input/counts/_m/astellas_dg_hg38_rseGene_n263.rda"), 
                     "transcripts"=here("input/counts/_m",
                                        "astellas_dg_hg38_rseTx_n263.rda"),
                     "exons"=here("input/counts/_m/astellas_dg_hg38_rseExon_n263.rda"),
    ...
    if(exists("rse_gene")){
        ...
        rownames(colData(rse_tx)) <- rnames[keepIdx]
...
get_local_ancestry <- function(feature, feature_id){
    afile = paste0("../../_m/dentateGyrus/",feature,"/ancestry/",
                   feature_id,"_AFR")
...
feature <- args$feature; n <- args$chunks; ID <- args$sge_id
                                        # Chunk data for loop
ids <- gsub("_AFR", "",
            list.files(paste0("../../_m/dentateGyrus/",feature,"/ancestry"),
                       pattern="\\_AFR$")) |> sort()

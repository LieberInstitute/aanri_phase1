...
prep_data <- function(feature){
    counts_lt = list("genes"=here("input/counts/_m",
                     ...
                     "transcripts"=here("input/counts/_m",
                                        paste0("caudate_brainseq_phase3_hg38_",
                                               "rseTx_merged_n464.rda")),
                     "exons"=here("input/counts/_m",
                                  paste0("caudate_brainseq_phase3_hg38_",
                                         "rseExon_merged_n464.rda")),
                     "junctions"=here("input/counts/_m",
                                      paste0("caudate_brainseq_phase3_hg38_",
                                             "rseJxn_merged_n464.rda")))
    ...
    if(exists("rse_gene")){
        ...
        counts  <- data.table::fread(tx_file) |>
            tibble::column_to_rownames("gencodeTx") |>
...
get_local_ancestry <- function(feature, feature_id){
    afile = paste0("../../_m/caudate/",feature,"/ancestry/",
                   feature_id,"_AFR")
...
eqtl_model <- function(feature, feature_id){
    ...
    print(paste("Samples (N):", dim(x$samples)[1]))
    # Design matrix
    ...
    # Load qSV
    qsv_file = here::here("input/phenotypes/_m/qSV_caudate.csv")
...
extract_de_loop <- function(feature, feature_id){
...
feature <- args$feature; n <- args$chunks; ID <- args$sge_id
                                        # Chunk data for loop
ids <- gsub("_AFR", "",
            list.files(paste0("../../_m/caudate/",feature,"/ancestry"),
                       pattern="\\_AFR$")) |> sort()

...
@lru_cache()
def get_hla_genes():
    df = pd.read_csv('../_m/hla_genes.csv', sep=',', usecols=[3])
    df["hla"] = 1
    return df
...
def merge_dataframe(tissue):
    df = pd.merge(get_hla_genes(), get_degs(),
                  on='gene_id', how="right")
    df["hla"] = df["hla"].fillna(0)
    return df[(df["region"] == tissue)].copy()
...
def cal_fishers_direction(direction, tissue):
    ...
    if direction == 'Up':
        ...
        df = df
    table = [[np.sum((df['lfsr']<0.05) & (df["hla"] == 1)),
              np.sum((df['lfsr']<0.05) & (df["hla"] == 0))],
             [np.sum((df['lfsr']>0.05) & (df["hla"] == 1)),
              np.sum((df['lfsr']>0.05) & (df["hla"] == 0))]]
    print(table)
...
def main():
    calculate_enrichment()\
        .to_csv('hla_enrichment_analysis.txt', sep='\t', index=False)

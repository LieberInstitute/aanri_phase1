...
def get_mash_lfsr(tissue):
    ...
    # get lfsr
    return pd.read_csv("../../_m/genes/mash_lfsr_local.txt", sep='\t')\
             .loc[:, ["feature_id", new_tissue]]\
             .rename(columns={new_tissue: "lfsr"})
...
def get_mash_es(tissue):
    if tissue == "Dentate Gyrus":
        ...
        new_tissue = tissue.lower()
    # get effect size
    return pd.read_csv("../../_m/genes/mash_effectsize_local.txt", sep='\t')\
             .loc[:, ["feature_id", new_tissue]]\
...
def annotate_mash(tissue):
    return get_mash_lfsr(tissue)\
        .merge(get_mash_es(tissue), on="feature_id")\
        .merge(get_annotation(), left_on="feature_id", right_on="names")\
...
def main():
    calculate_enrichment()\
        .to_csv('celltype_enrichment_analysis.txt', sep='\t', index=False)

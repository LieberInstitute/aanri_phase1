...
@lru_cache()
def get_local_sig(feature):
    return get_local_mash(feature)[(get_local_mash(feature)["lfsr"] < 0.05)].copy()


@lru_cache()
def prepare_local_data(feature, tissue1, tissue2):
    df1 = get_local_sig(feature)[(get_local_sig(feature)["region"] == tissue1)].copy()
    df2 = get_local_sig(feature)[(get_local_sig(feature)["region"] == tissue2)].copy()
    sig_genes = set(df1.index) | set(df2.index)
    df = get_local_mash(feature).loc[sig_genes, ["region", "posterior_mean"]]\
                               .reset_index()
...
@lru_cache()
def prepare_local(feature, tissue):
...
@lru_cache()
def prepare_local_shared(feature, tissue):
    df1 = get_mash(feature).loc[(get_mash(feature)["Tissue"] == tissue),
                                ["lfsr", "posterior_mean"]]\
                           .rename(columns={"posterior_mean":"continous"}).copy()
    df2 = get_local_mash(feature).loc[(get_local_mash(feature)["region"] == tissue),
...
def corr_beta(tissue1, tissue2):
    df = prepare_local_data("Gene", tissue1, tissue2)
    return spearmanr(df[tissue1], df[tissue2])


def corr_beta_local(fnc, tissue, feature):
    df = fnc(feature, tissue)
    return spearmanr(df["continous"], df["local"])


def plotNsave_corr(tissue1, tissue2):
    pandas2ri.activate()
    globalenv['df'] = prepare_local_data("Gene", tissue1, tissue2)
...
def plotNsave_corr_local(fnc, tissue, label, feature):
    pandas2ri.activate()
    ...
    xlab = paste0("Effect Size\n(Global Ancestry)")
    ylab = paste0("Effect Size\n(Local Ancestry)")
    fn = paste(label, "local_effectsize_scatter",
               gsub(" ", "_", tissue), sep="_")
    pp = ggpubr::ggscatter(df, x="continous", y="local", add="reg.line", size=1,
...
def main():
    ## Comparison
    config = {'either': prepare_local,
              'shared': prepare_local_shared}
    with open("rho_statistics_local.log", "w") as f:
        for feature in ["Gene", "Transcript", "Exon", "Junction"]:
            ...
            for label in ["either", "shared"]:
                ...
                for tissue in ["Caudate", "Dentate Gyrus", "DLPFC", "Hippocampus"]:
                    ## Generate figure
                    plotNsave_corr_local(config[label], tissue, label, feature)
                    ## Correlated effect sizes
                    rho, pval = corr_beta_local(config[label], tissue, feature)
                    print(f"Local ({tissue}):\t\t r2 ~ {rho**2:.3f}\t"+\

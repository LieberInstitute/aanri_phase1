...
get_sig_genes <- function(){
    ...
    return(data.table::fread(fn) |> dplyr::filter(feature_type == "Gene"))
}
memGENES <- memoise::memoise(get_sig_genes)

prep_heatmap <- function(){
    caudate <- memGENES() |> dplyr::filter(region == "Caudate")
    gyrus <- memGENES() |> dplyr::filter(region == "Dentate Gyrus")
    dlpfc <- memGENES() |> dplyr::filter(region == "DLPFC")
    hippo <- memGENES() |> dplyr::filter(region == "Hippocampus")
    lt = list(Caudate = caudate$feature_id, `Dentate Gyrus`=gyrus$feature_id,
              DLPFC=dlpfc$feature_id, Hippocampus=hippo$feature_id)
    m = make_comb_mat(lt)
    return(m)
}
...
plot_upset <- function(){
    ...
    top_annot = upset_top_annotation(
        ...
        annotation_name_rot = 90)
                                        # Plotting
    pdf('BrainSeq_ancestry_upsetR_mash.pdf', width=8, height=4)
    ht = draw(UpSet(m, pt_size=unit(4, "mm"), lwd=3,
                    comb_col=cbb_palette[comb_degree(m)],
                    set_order = c("Caudate", "Dentate Gyrus",
                                  "DLPFC", "Hippocampus"),
...
plot_upset_transposed <- function(){
    ...
    top_ha = HeatmapAnnotation(
        ...
        annotation_name_rot = 90)
                                        # Plotting
    pdf('BrainSeq_ancestry_upsetR_mash_tranposed.pdf', width=5, height=10)
